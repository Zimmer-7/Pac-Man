...
public class EnemyBlue extends Enemy{
	...
	public EnemyBlue(double x, double y, int width, int height, BufferedImage sprite) {
		super(x, y, width, height, sprite);
		
		enemyRight = new BufferedImage[2];
		enemyLeft = new BufferedImage[2];
		
		enemyLeft[0] = Game.spriteSheet.getSprite(0, 48, 16, 16);
		enemyLeft[1] = Game.spriteSheet.getSprite(0, 64, 16, 16);
		enemyRight[0] = Game.spriteSheet.getSprite(16, 48, 16, 16);
		enemyRight[1] = Game.spriteSheet.getSprite(16, 64, 16, 16);
	}
	...
	public void tick() {
		if(!ghost && !dead){
			...
			if(countRandom == maxCount) {
				random = Game.rand.nextInt(100);
				countRandom = 0;
			}
			
			if(path == null || path.isEmpty() || ghostFrames == ghostMaxFrames || random > 80) {
				ghostFrames = 0;
